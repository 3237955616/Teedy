...
public class UserDao {
     ...
     */
    public void createUserFromRegistration(RegistrationRequest request) {
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        
        // Check if username is available
        if (getActiveByUsername(request.getUsername()) != null) {
            throw new RuntimeException("Username already exists: " + request.getUsername());
        }

        // Hash password with BCrypt
        String hashedPassword = BCrypt.withDefaults().hashToString(Constants.DEFAULT_BCRYPT_WORK, request.getPassword().toCharArray());

        // Create new user
        User user = new User();
        user.setId(request.getUserId());  // Assuming request contains a generated ID
        user.setUsername(request.getUsername());
        user.setEmail(request.getEmail());
        user.setPassword(hashedPassword);
        user.setRoleId(Constants.DEFAULT_USER_ROLE);  // Assign default role
        user.setCreateDate(new Date());
        user.setFirstConnection(true);
        user.setPrivatekey(request.getPrivateKey());  // If private key is part of request
        user.setStatus("PENDING");  // Set initial status to pending approval
    ...
    public User getActiveByUsername(String username) {
        try {
            return (User) entityManager.createQuery("SELECT u FROM User u WHERE u.username = :username AND u.status = 'APPROVED'")
     ...
     */
    public void rejectRegistration(String userId) {
        User user = getUserById(userId);
        if (user != null && "PENDING".equals(user.getStatus())) {
